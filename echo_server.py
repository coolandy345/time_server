...
def handle(client_socket,addr):
    ...
    print('[{}] {} - handle connection, start - {}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'), threading.current_thread().name, remote_addr))
    ...
    print('[{}] {} - handle connection, exit - {}'.format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'), threading.current_thread().name, remote_addr))
    ...
    with ThreadPoolExecutor(max_workers = workers) as executor:
        ...
        try:
            while True:
                ...
                executor.submit(handle, client_socket,addr)
